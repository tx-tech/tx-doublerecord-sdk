...
import com.common.widget.view.SimpleLayout;
import com.common.widget.aciton.ActivityAction;
import com.txt.sl.R;
...
public final class PlayerView extends SimpleLayout
    ...
    public PlayerView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        LayoutInflater.from(getContext()).inflate(R.layout.tx_widget_player_view, this, true);
...
//    }
    ...
    public void setVideoSource(String url,boolean isNet) {
    ...
    public void start() {
        mVideoView.start();
        mControlView.setImageResource(R.drawable.tx_video_play_pause_ic);
        // 延迟隐藏控制面板
    ...
    public void pause() {
        mVideoView.pause();
        mControlView.setImageResource(R.drawable.tx_video_play_start_ic);
        // 延迟隐藏控制面板
        removeCallbacks(mHideControllerRunnable);
        postDelayed(mHideControllerRunnable, CONTROLLER_TIME);
    }
    ...
    public void lock() {
        mLockMode = true;
        mLockView.setImageResource(R.drawable.tx_video_lock_close_ic);
    ...
    public void unlock() {
        mLockMode = false;
        mLockView.setImageResource(R.drawable.tx_video_lock_open_ic);
    ...
    public boolean onInfo(MediaPlayer player, int what, int extra) {
        switch (what) {
            ...
            case MediaPlayer.MEDIA_INFO_BUFFERING_START:
                mMessageView.setText(R.string.tx_common_loading);
...
//                        mLottieView.setImageResource(mAdjustSecond < 0 ? R.drawable.video_schedule_rewind_ic : R.drawable.video_schedule_forward_ic);
                ...
                if (mTouchOrientation == LinearLayout.VERTICAL) {
                    ...
                    if ((int) event.getX() < getWidth() / 2) {
                        ...
                        if (percent > 100 / 3 * 2) {
                            iconId = R.drawable.tx_video_brightness_high_ic;
                        } else if (percent > 100 / 3) {
                            iconId = R.drawable.tx_video_brightness_medium_ic;
                        } else {
                            iconId = R.drawable.tx_video_brightness_low_ic;
                        }
//                        mLottieView.setImageResource(iconId);
                    ...
                    @DrawableRes int iconId;
                    if (percent > 100 / 3 * 2) {
                        iconId = R.drawable.tx_video_volume_high_ic;
                    } else if (percent > 100 / 3) {
                        iconId = R.drawable.tx_video_volume_medium_ic;
                    } else if (percent != 0) {
                        iconId = R.drawable.tx_video_volume_low_ic;
                    } else {
                        iconId = R.drawable.tx_video_volume_mute_ic;
                    }
//                    mLottieView.setImageResource(iconId);
                    mMessageView.setText(String.format("%s %%", percent));
                    post(mShowMessageRunnable);
