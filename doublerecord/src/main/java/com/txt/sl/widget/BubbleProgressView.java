...
public class BubbleProgressView extends View {
    ...
    private int mColorProgressStr = Color.RED;//进度条的进度文字的颜色
    private float mProgressHeight = 10;
    private float mProgress=0;//进度条的进度
    private float mBubbleTriangleHeight = 15;//气泡底部小三角高度
    private float mBubbleRectRound = 5;//气泡的圆角
    private String mProgressStr = "0%";//显示进度的字符串
    private float mTextSize = 40;//进度条文字大小
    private Paint.FontMetricsInt mFontMetricsInt;
    private float mProgressStrMargin = 30;//气泡的边距
    public boolean mShowBubble = false;
    ...
    private void init() {
        ...
        mPaintProgressStr.setStyle(Paint.Style.FILL_AND_STROKE);
        ...
        mColorProgressStr = getResources().getColor(R.color.color_FA6400);
        ...
        mColorProgressBg = getResources().getColor(R.color.color_ECECEC);
        mColorProgress = getResources().getColor(R.color.red);
        mPaintBubble.setColor(getResources().getColor(R.color.color_FFECEC));//设置气泡的颜色
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mPathSrc.moveTo(0,0);
        mPathSrc.lineTo(w,0);//进度条位置在控件整体底部，且距离控件左边和右边各20像素
    ...
    private void drawBubble(Canvas canvas) {
        ...
        RectF rectF = new RectF(p[0]+mBubbleTriangleHeight+mBubbleRectRound/2+((1-mProgress)*width),
