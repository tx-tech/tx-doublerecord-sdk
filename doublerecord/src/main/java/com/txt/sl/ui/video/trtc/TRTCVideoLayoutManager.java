 ...
 * <p>
 * 3.堆叠布局：{@link TRTCVideoLayoutManager#makeGirdLayout(boolean)} 思路是初始化一系列的 x、y、padding、margin 组合 LayoutParams 直接对 View 进行定位
 * <p>
...
//        return null;
    ...
    public void makeGirdLayout(boolean needUpdate) {
        if (mGrid4ParamList == null || mGrid4ParamList.size() == 0 || mGrid9ParamList == null || mGrid9ParamList.size() == 0) {
            mGrid4ParamList = RoomVideoUiUtils.initLocalView(getContext(), getWidth(), getHeight());
        }
        if (needUpdate) {
