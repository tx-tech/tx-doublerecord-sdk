...
class TXManagerImpl : ITXManager {
    ...
    //跳转到详情页面
    override fun gotoOrderDetaisPage(context: Activity?, loginName: String, fullName: String, orgCode: String, flowId: String, sign: String, listener: onCreateRoomListener
    ) {
        ...
        freeLogin(orgCode, sign, loginName, fullName, object : netResultCallBack {
            override fun onResultSuccess(result: String) {
                ...
                if (getAgentId().isEmpty() || getTenantId().isEmpty() || getToken().isEmpty()) {
                    ...
                    OrderDetailsPageActivity.newActivity(context!!,flowId)
                }
    ...
    //跳转到创建工单页面
    override fun gotoCreateDetaisPage(context: Activity?, loginName: String, fullName: String, orgCode: String, sign: String, listener: onCreateRoomListener
    ) {
        this.mLoginName = loginName
        freeLogin(orgCode, sign, loginName, fullName, object : netResultCallBack {
            override fun onResultSuccess(result: String) {
    ...
    //跳转到工单列表页面
    override fun gotoOrderListPage(context: Activity?, loginName: String, fullName: String, orgCode: String, sign: String, listener: onCreateRoomListener
    ) {
        this.mLoginName = loginName
        freeLogin(orgCode, sign, loginName, fullName, object : netResultCallBack {
            override fun onResultSuccess(result: String) {
    ...
    //跳转到上传视频页面
    override fun gotoVideoUploadPage(context: Activity?, loginName: String, fullName: String, orgCode: String, flowId:String,sign: String, listener: onCreateRoomListener) {
        this.mLoginName = loginName
        freeLogin(orgCode, sign, loginName, fullName, object : netResultCallBack {
            override fun onResultSuccess(result: String) {
                val stringStr = GsonUtils.getJson(TXSdk.getInstance().application, "reportstates.json")
                ...
                if (getAgentId()!!.isEmpty() || getTenantId()!!.isEmpty() || getToken()!!.isEmpty()) {
                    ...
                    VideoUploadActivity.newActivity(context!!,flowId)
                }
